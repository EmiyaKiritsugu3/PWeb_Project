...
export const WorkoutGeneratorInputSchema = z.object({
  ...
  observacoesAdicionais: z
    ...
    .optional()
    .describe(
      'Qualquer observação adicional do instrutor, como lesões pré-existentes ou preferências do aluno.'
    ),
});
...
const ExercicioGeradoAISchema = z.object({
  nomeExercicio: z
    .string()
    .describe('O nome exato do exercício como fornecido na lista de exercícios disponíveis.'),
  grupoMuscular: z
    .string()
    .describe('O nome exato do grupo muscular ao qual este exercício pertence.'),
  ...
  observacoes: z
    .string()
    .describe(
      "Observações curtas e úteis para o aluno (ex: 'Controlar a descida', 'Não travar os joelhos'). Deixe em branco se não houver observação especial."
    ),
});
...
const TreinoGeradoAISchema = z.object({
  ...
  diaSugerido: z
    ...
    .min(0)
    .max(6)
    .describe('O dia da semana sugerido para este treino (0=Domingo, 1=Segunda, ..., 6=Sábado).'),
