        ...
        try {
            ...
            for (const workout of result.workouts) {
                const novosExercicios = workout.exercicios.map((ex: any) => ({
                    ...
                    descricao: EXERCICIOS_POR_GRUPO.flatMap(g => g.exercicios).find(ez => ez.nomeExercicio === ex.nomeExercicio)?.descricao || ""
                }));
                    ...
                    <div key={treino.id} className={cn("rounded-lg border p-4 transition-all flex flex-col sm:flex-row justify-between sm:items-center gap-4", treino.diaSemana !== null && "bg-accent/10 border-accent")}>
                       <div className='flex-1'>
                            <div className='flex items-center gap-3'>
                                ...
                                {treino.instrutorId && treino.instrutorId !== userId && <Badge variant="secondary">Do Personal</Badge>}
                            </div>
