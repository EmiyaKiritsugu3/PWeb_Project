...
export default function TreinosManagementClient({ initialAlunos, instrutorId }: { initialAlunos: Aluno[], instrutorId: string }) {
    ...
    const handleSaveTreino = async () => {
        if (!selectedAlunoId || !objetivo || exercicios.length === 0 || exercicios.some(e => !e.nomeExercicio)) {
            toast({ title: "Erro ao salvar", description: "Verifique os campos obrigatórios.", variant: "destructive" });
            return;
        }
                        ...
                        <Card>
                            ...
                            <CardContent className="grid gap-6">
                                <div className='grid gap-2'>
                                    ...
                                    <Input id="objetivo" placeholder="Ex: Peito e Tríceps" value={objetivo} onChange={(e) => setObjetivo(e.target.value)} />
                                </div>

                                <div className="grid gap-4">
                                    <h3 className='font-medium'>Exercícios</h3>
                                    {exercicios.map((exercicio, index) => (
                                        <div key={exercicio.id} className="grid grid-cols-1 md:grid-cols-[1fr_auto_auto_1fr_auto] items-end gap-3 rounded-md border p-4 bg-muted/20">
                                            <div className="grid gap-2">
                                            ...
                                            <Button variant="ghost" size="icon" onClick={() => handleRemoveExercicio(exercicio.id!)}>
