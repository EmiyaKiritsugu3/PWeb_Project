...
import {
  ...
  finalizarTreinoAction,
} from '../src/lib/actions/alunos';
...
async function runTest() {
  ...
  try {
    ...
    if (!treinoResult.success || !treinoResult.data) {
      console.warn(
        `   Aviso: finalizarTreinoAction falhou (provavelmente falta de sessão Supabase no CLI): ${treinoResult.error}`
      );
    } else {
    ...
    console.log(
      `   XP Atual: ${finalAluno?.exp}, Nível: ${finalAluno?.nivel}, Streak: ${finalAluno?.streakDiasSeguidos}`
    );
