...
## 7. Detalhamento Técnico Complementar

### 7.1. Visão de Fluxo Full-Stack
A solução utiliza o framework **Next.js 15** com **Server Actions** para garantir que a lógica de negócio permaneça no servidor, reduzindo a exposição de APIs e melhorando a segurança.

### 7.2. Orquestração de IA
A inteligência artificial é integrada via **Google Genkit**, que padroniza o consumo de LLMs (como o Gemini 1.5 Pro/Flash). Isso permite:
- **Geração de Treinos:** Prompts estruturados que consideram biotipo, nível e objetivo.
- **Feedback Adaptativo:** Mensagens motivacionais baseadas no histórico recente do aluno.

### 7.3. Infraestrutura de Dados
- **Banco de Dados:** PostgreSQL hospedado no **Supabase**.
- **Conectividade:** Uso de **PgBouncer** (Transaction Mode) para gerenciar o pool de conexões em ambiente serverless.
- **Migrations:** Controladas via **Prisma CLI**, garantindo versionamento do esquema.

### 7.4. Estratégia de Deploy
- **Plataforma:** Vercel (Edge Runtime e Serverless Functions).
- **CI/CD:** Pipelines automatizados que validam o build e tipos antes de cada deploy em produção.
