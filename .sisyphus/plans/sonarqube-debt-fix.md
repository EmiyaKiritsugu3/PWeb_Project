...
- All 1070 tests must continue passing
...
- `data-ai-hint` non-standard attribute — **JÁ REMOVIDO** (não existe mais no codebase)
...
- `data-ai-hint` — **JÁ REMOVIDO** do codebase
- Skeleton array keys are false positives — only fix data list keys
- aria-labels must be in Portuguese (matching UI language)
- `src/components/dashboard/alunos/columns.tsx:36` já tem aria-label — verificar antes de fixar
...
- `src/components/dashboard/alunos/columns.tsx` — **aria-label já existe (line 36)**
- ~~`src/app/aluno/layout.tsx` — data-ai-hint handled~~ **REMOVIDO**
...
- NO new tests added
- NO files touched outside the 7 identified files
...
Steps: 1. Run: grep -n "flatMap\|reduce" src/lib/actions/treinos.ts 2. Verify no inline flatMap→map or reduce→filter chains remain in registrarHistoricoTreinoAction 3. Run: npx tsc --noEmit
...
Steps: 1. Run: npm test 2. Verify output shows "1070 tests passed" (or equivalent)
...
- Read these files and identify the exact key usage:
  ...
  - `src/app/dashboard/treinos/treinos-client.tsx:92,106` — data list keys
...
- For **data list keys** (treinos-client.tsx:92,106): Replace `${treino.nome}-${treinoIndex}` with `treino.id` if available, or keep the composite key if it's already stable
...
- `src/components/ui/dashboard-skeletons.tsx:19` — Skeleton key
- `src/app/dashboard/treinos/treinos-client.tsx:92,106` — Data list keys (treino and exercicio)
...
Steps: 1. Run: grep -n "key={\`.\*\$\{" src/app/dashboard/treinos/treinos-client.tsx 2. Verify keys use stable identifiers (treino.id, exercicio.id) or stable composite keys 3. Run: npm test
...
Tool: Bash
Preconditions: Changes applied
Steps: 1. Run: grep -rn "key={\`skeleton-" src/ 2. Verify either sonar-ignore comments added or keys stabilized
...
- Read these files and add `aria-label` to icon-only buttons:
  ...
  - `src/app/aluno/aluno-header.tsx:77` — User avatar button (dropdown trigger) — **JÁ TEM** `aria-label="Perfil do usuario"` ✅
...
Steps: 1. Run: grep -n "aria-label" src/components/dashboard/alunos/columns.tsx src/app/dashboard/treinos/treinos-client.tsx src/app/aluno/aluno-header.tsx 2. Verify aria-labels are present and in Portuguese 3. Run: npm test
...
- Files: `src/components/dashboard/alunos/columns.tsx`, `src/app/dashboard/treinos/treinos-client.tsx`, `src/app/aluno/aluno-header.tsx`
- Pre-commit: `npm run pre-flight`

---

- [x] 4. Handle data-ai-hint Non-Standard Attribute (1 MAJOR issue) — **JÁ REMOVIDO** ✅

**What to do**:
- `grep -rn "data-ai-hint" src/` confirmado: **não existe mais no codebase**
- Atributo já foi removido anteriormente (provavelmente no PR #128 ou anterior)
...
Steps: 1. Run: grep -rn "data-ai-hint" src/ (before fix — check consumption) 2. If consumed: verify suppression comment added 3. If NOT consumed: verify attribute removed 4. Run: npm test
...
Steps: 1. Run: npm run pre-flight 2. Verify exit code 0 3. Check output for "1070 tests passed" or equivalent
