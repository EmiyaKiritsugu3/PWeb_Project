...
Tool: Bash
Steps: 1. Run: grep -r 'placeholder-images' src/ --include='_.{ts,tsx,js,json}' > .sisyphus/evidence/wave0-imports.txt 2. Run: grep -r 'STREAK_MULTIPLIER\|BONUS_THRESHOLD' src/ --include='_.{ts,tsx}' > .sisyphus/evidence/wave0-constants.txt 3. Run: npm run test 2>&1 | tee .sisyphus/evidence/wave0-baseline.txt
...
Tool: Bash
Steps: 1. Run: npx tsc --noEmit src/lib/dummyDb.ts src/services/alunoService.ts 2>&1 | tee .sisyphus/evidence/task2-typecheck.txt
Expected Result: 0 errors
Evidence: .sisyphus/evidence/task2-typecheck.txt

Scenario: alunoService returns typed result
Steps: 1. Check that alunoService.ts has no unchecked return of db methods 2. Run: grep -rn ': Promise<unknown>' src/services/alunoService.ts
...
Tool: Bash
Steps: 1. Run: npx tsc --noEmit src/components/dashboard/alunos/columns.tsx 2>&1
Expected Result: 0 errors
Evidence: .sisyphus/evidence/task3-typecheck.txt

Scenario: No bare getValue calls
Steps: 1. grep -n 'row\.getValue(' src/components/dashboard/alunos/columns.tsx
...
Tool: Bash
Steps: 1. Run: npx tsc --noEmit src/ai/flows/workout-generator-flow.ts 2>&1
Expected Result: 0 errors
Evidence: .sisyphus/evidence/task4-typecheck.txt

Scenario: Null check present
Steps: 1. grep -n 'if (!output)' src/ai/flows/workout-generator-flow.ts
...
Tool: Bash
Steps: 1. Create test file or add unit test verifying: given localStorage 'pt-BR',
...
Steps: 1. Verify: given localStorage 'PT-BR' or 'Pt-bR',
the language state is 'pt'
...
Tool: Bash
Steps: 1. Run vitest test: handleActionError with ZodError, no customMessage
Expected Result: Returns 'Dados inválidos'
Evidence: .sisyphus/evidence/task6-default.txt

Scenario: Custom message
Steps: 1. Run vitest test: handleActionError with ZodError, customMessage='Custom error'
...
Tool: Bash
Steps: 1. Run: grep -n 'as Record<string, unknown>' src/lib/logger.ts
Expected Result: 0 matches
Evidence: .sisyphus/evidence/task7-nocasts.txt

Scenario: Circular object safety
Tool: Bash
Steps: 1. Create a one-off test with circular object: const a: any = { b: 1 }; a.self = a; 2. Call Logger.info('test', a)
...
Tool: Bash
Steps: 1. grep -rn 'STREAK_MULTIPLIER\|BONUS_THRESHOLD' src/ --include='\*.{ts,tsx}'
Expected Result: 0 matches
Evidence: .sisyphus/evidence/task9-old-names.txt

Scenario: New names present
Steps: 1. grep -n 'GROWTH_BASE_FACTOR\|GROWTH_INCREMENT' src/lib/data.ts
