  ...
  **QA Scenarios (MANDATORY)**:
  ...
  Scenario: globalSetup runs seed successfully on fresh DB
    ...
    Steps:
      ...
      2. npx tsx -e "import {PrismaClient} from '@prisma/client'; const p=new PrismaClient(); p.aluno.count({where:{statusMatricula:'INADIMPLENTE'}}).then(n=>{console.log('before:',n);process.exit(0)})"
         → Expected: "before: 0"
      3. npx tsx tests/e2e/global-setup.ts
         → Expected: exit 0, output contains "✅ E2E seed complete" and seed's own "E2E seed complete."
      4. npx tsx -e "import {PrismaClient} from '@prisma/client'; const p=new PrismaClient(); p.aluno.count({where:{statusMatricula:'INADIMPLENTE'}}).then(n=>{console.log('after:',n);process.exit(0)})"
         → Expected: "after: 1"
  ...
  Scenario: globalSetup fails loud when env vars missing
    ...
    Steps:
      1. unset DATABASE_URL NEXT_PUBLIC_SUPABASE_URL SUPABASE_SERVICE_ROLE_KEY
      2. npx tsx tests/e2e/global-setup.ts
  ...
  Scenario: Playwright config loads without error
    ...
    Steps:
      1. npx playwright test --list
         → Expected: lists all 9 spec files (auth, enrollment, financial-access, instrutor-auth-negative, instrutor-workflow, nav-visibility, payment-status, student-portal, workout-session)
      2. echo "exit: $?"
         → Expected: exit: 0
...
- [x] 3. Rename env var in `.env.test`
  ...
  **QA Scenarios (MANDATORY)**:
  ...
  Scenario: .env.test loads new env var name correctly
    ...
    Steps:
      1. dotenv -e .env.test -- node -e "console.log('new:', process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_DEFAULT_KEY ? 'SET' : 'EMPTY')"
         → Expected: "new: SET"
      2. dotenv -e .env.test -- node -e "console.log('old:', process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || 'UNSET')"
         → Expected: "old: UNSET"
      3. dotenv -e .env.test -- node -e "console.log('len:', process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_DEFAULT_KEY.length)"
...
- [x] 4. Add new env var to `.env.local` (additive — keep old)
  ...
  **QA Scenarios (MANDATORY)**:
  ...
  Scenario: .env.local has both old and new env var names set
    ...
    Steps:
      1. dotenv -e .env.local -- node -e "const o=process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY; const n=process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_DEFAULT_KEY; console.log('old:', o ? o.length+'chars' : 'EMPTY'); console.log('new:', n ? n.length+'chars' : 'EMPTY'); console.log('match:', o===n)"
...
- [x] 5. Close debt entry in `docs/process/TECHNICAL-DEBT.md`
  ...
  **QA Scenarios (MANDATORY)**:
  ...
  Scenario: TECHNICAL-DEBT entry is marked resolved
    ...
    Steps:
      1. grep -A 2 "Falha de Seed no E2E" docs/process/TECHNICAL-DEBT.md
         → Expected: shows entry text + new "Status: ✅ Resolved" or similar marker
      2. grep -c "^### 2\." docs/process/TECHNICAL-DEBT.md
...
- [x] 6. Add CHANGELOG entry
  ...
  **QA Scenarios (MANDATORY)**:
  ...
  Scenario: CHANGELOG entry follows repo convention
    ...
    Steps:
      1. head -30 CHANGELOG.md
         → Expected: shows new entry with "### Fixed" subsection at top
      2. grep -c "auto-seed before Playwright" CHANGELOG.md
         → Expected: 1
      3. grep -c "NEXT_PUBLIC_SUPABASE_PUBLISHABLE_DEFAULT_KEY" CHANGELOG.md
         → Expected: ≥ 1
...
- [x] 7. Local e2e verification + pre-flight (Wave 2 gate)
  ...
  **QA Scenarios (MANDATORY)**:
  ...
  Scenario: Originally-failing payment-status test now passes
    ...
    Steps:
      1. npm run e2e -- --grep "GERENTE registers payment" 2>&1 | tee .sisyphus/evidence/fix-payment-status-e2e/task-7-targeted.txt
         → Expected: 1 passed (chromium)
      2. cat test-results/.last-run.json
  ...
  Scenario: Full e2e suite passes (no regression in other specs)
    ...
    Steps:
      1. npm run e2e 2>&1 | tee .sisyphus/evidence/fix-payment-status-e2e/task-7-full-suite.txt
  ...
  Scenario: Pre-flight gates all green
    ...
    Steps:
      1. npm run pre-flight 2>&1 | tee .sisyphus/evidence/fix-payment-status-e2e/task-7-preflight.txt
  ...
  Scenario: Third run is idempotent (no test pollution)
    ...
    Steps:
      1. npm run e2e 2>&1 | tee .sisyphus/evidence/fix-payment-status-e2e/task-7-idempotent.txt
  ...
  **QA Scenarios (MANDATORY)**:
  ...
  Scenario: Single atomic commit with all 6 changes
    ...
    Steps:
      ...
      2. git log --oneline -2
         → Expected: HEAD = "fix(e2e): auto-seed before Playwright + align .env.test to Supabase SSR 0.10+", HEAD~1 = main HEAD
      3. git show --stat HEAD
         → Expected: 6 files (5 modified + 1 new global-setup.ts)
      4. git show HEAD --name-only | grep -E "test-results|playwright-report"
...
- [x] 9. Push branch + open PR against `main`
  ...
  **QA Scenarios (MANDATORY)**:
  ...
  Scenario: PR opened successfully with correct metadata
    ...
    Steps:
      1. git push -u origin fix/payment-status-e2e-auto-seed
         → Expected: branch published, no errors
      2. gh pr create --base main --head fix/payment-status-e2e-auto-seed --title "fix(e2e): auto-seed before Playwright + align .env.test to Supabase SSR 0.10+" --body "..." 2>&1 | tee .sisyphus/evidence/fix-payment-status-e2e/task-9-pr-create.txt
         → Expected: PR URL printed, e.g. "https://github.com/EmiyaKiritsugu3/PWeb_Project/pull/<N>"
      3. gh pr view --json title,baseRefName,headRefName,url,body
