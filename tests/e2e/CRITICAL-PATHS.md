...
### Selector Patterns (discovered across It2–It5)
...
| `getByText` on shared vocabulary   | `page.getByText('INADIMPLENTE')` matches heading, description, and badge — 5 elements                    | Define row first: `const row = page.getByRole('row', { name: /.../ })`, then `row.getByText('INADIMPLENTE', { exact: true })` | `getByText` does substring matching; page headings include the same word; strict mode throws          |
| Toast assertion strict mode        | `page.getByText('Pagamento Registrado!')` matches 2 elements: `<li role="status">` AND its `<div>` child | `page.getByRole('status').filter({ hasText: 'Pagamento Registrado!' })`                                                       | Radix Toast renders text in nested elements; `getByText` matches both parent container and text child |
...
- **Seed `update: {}`** deixa estado mutado por testes persistir entre re-runs. Sempre espelhar campos mutáveis no `update` do `upsert` (ex: `status`, `dataVencimento`, `statusMatricula`).
